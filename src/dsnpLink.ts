...
export type CreatePostRequestV2 = {
    content: string;
    inReplyTo?: string;
    assets?: string[];
};
 ...
 * Create a new post
 */
export async function createBroadcastV2<FetcherData>(ctx: r.Context<AuthMethods, FetcherData>, params: {}, body: CreatePostRequestV2, opts?: FetcherData): Promise<BroadcastExtended> {
    const req = await ctx.createRequest({
        path: '/v2/content',
        params,
        method: r.HttpMethod.POST,
 ...
 * Create a new post
 */
export async function uploadAsset<FetcherData>(ctx: r.Context<AuthMethods, FetcherData>, params: {}, body: any, opts?: FetcherData): Promise<UploadAssetResponse> {
    const req = await ctx.createRequest({
        path: '/v1/assets',
        params,
        method: r.HttpMethod.POST,
