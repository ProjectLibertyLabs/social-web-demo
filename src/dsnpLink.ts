...
export type VerifiableCredentialWithEd25519Proof =
  VerifiableCredentialWithoutProof;
export type InteractionTicket = VerifiableCredentialWithEd25519Proof;
 ...
 */
export async function authAssert<FetcherData>(
  ctx: r.Context<AuthMethods, FetcherData>,
  params: {},
  opts?: FetcherData,
): Promise<any> {
 ...
 */
export async function getInteractionsFeed<FetcherData>(
  ctx: r.Context<AuthMethods, FetcherData>,
  params: {
    newestBlockNumber?: number;
    oldestBlockNumber?: number;
  ...
  const req = await ctx.createRequest({
    ...
    params,
    method: r.HttpMethod.GET,
    queryParams: ["newestBlockNumber", "oldestBlockNumber", "rel", "href"],
  });
  const res = await ctx.sendRequest(req, opts);
  return ctx.handleResponse(res, {});
}
 ...
 */
export async function submitInteraction<FetcherData>(
  ctx: r.Context<AuthMethods, FetcherData>,
  params: {},
